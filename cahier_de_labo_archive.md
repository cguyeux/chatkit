...
Entrées anciennes déplacées ici pour n'alléger que la lecture courante de `cahier_de_labo.md`. Contenu verbatim, jamais réécrit ni condensé.
Recherche : `rg "Pxx" cahier_de_labo_archive.md` (numéro de piste), `rg -i "<mot-clé>"`, ou `rg "^## AAAA-MM"` pour une plage de dates.
